...
class Scheduler extends Component {
  ...
  bookTimeSlot = (currentDate,currentPC)=>{
    debugger
    if(!this.finalBookedArray){
      alert("Please select any time slot to book");
    }
    axios.get("http://localhost:3000/bookTimeSlot?bookids="+this.finalBookedArray.concat(this.initialBookedTime)+"&cDate="+currentDate+"&cPID="+currentPC+"&id=1")
    .then((response) => {
        console.log(response.data);
