...
app.get('/getAllusers', (req, res) => {
  //Select all customers and return the result object:
  var selectQuery="SELECT * FROM slot_login";
  con.query(selectQuery, function (err, result, fields) {
    if (err) throw err;
    res.setHeader('Content-Type', 'application/json');
    if(result.length > 0){
      res.end(JSON.stringify({ users: result, statusCode:200}));
    }
    else{
      res.status(400);
      res.end(JSON.stringify({ a: "Error Retrieving", statusCode:500}));
    }      
  });
})
...
app.get('/updateUserRole', (req, res) => {
  ...
  var totalObj=req.query.bookids.split(":::");
  var username=totalObj[0];
  var userRole=totalObj[1];

  var updateQuery = "UPDATE slot_login set role='"+username.toString() +"' WHERE username="+username.toString()
