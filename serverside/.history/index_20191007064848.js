...
app.get('/userSignIn', (req, res) => {
  ...
  var usernamr=totalObj[0];
  var password=totalObj[1];
  var selectQuery="SELECT * FROM slot_login where `username`='"+usernamr+"' AND `password`='"+password+"'";
  ...
  con.query(selectQuery, function (err, result, fields) {
    ...
    if(result.length>-0)
      res.end(JSON.stringify({ a: "Sign In Up Done Successfully", statusCode:200}));
    else
      res.end(JSON.stringify({ a: "User Already Exists in DataBase", statusCode:400}));
  });
})
...
app.get('/userSignUp', (req, res) => {
      ...
      var password=totalObj[1];
      var sqlQuery = 'INSERT INTO slot_booking (`userID`,`username`,`password`) VALUES ("'+username.toString()+'","'+password.toString()+'")'
      console.log(selectQuery)
      con.query(selectQuery, function (err, result, fields) {
        if (err) throw err;
        res.setHeader('Content-Type', 'application/json');
